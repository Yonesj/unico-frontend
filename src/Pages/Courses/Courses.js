...
const Courses = (children) => {
  ...
  useEffect(() => {
    ...
    const formatted = saved.map((c, i) => {
      ...
      return {
        ...
        gender:
          c.gender == "-" ? "-" : c.gender === "F" ? "خواهران" : "برادران",
        time: c.classes ? (
          <div>
            {c.classes.map((time, index) => (
              <p key={index}>
                {dayMap[time.day]} {time.start} - {time.end}
              </p>
            ))}
          </div>
        ) : (
  ...
  const filteredData = useMemo(() => {
    return dataSource.filter((item) => {
      const courseText =
        item.courseName.props?.children?.[0]?.props?.children?.toString() || "";

      const search = normalizeText(searchText);
      return normalizeText(courseText).includes(search);
    });
  ...
  return (
    <DndContext modifiers={[restrictToVerticalAxis]} onDragEnd={onDragEnd}>
            ...
            <div className="flex items-center justify-evenly gap-3 text-sm ">
              <div className="flex border border-solid border-[#D9D9D9] overflow-auto rounded-lg font-normal text-sm">
                ...
                >
                  ...
                  />
                  <path
                    d="M28 28L24.1333 24.1333M26.2222 19.1111C26.2222 23.0385 23.0385 26.2222 19.1111 26.2222C15.1838 26.2222 12 23.0385 12 19.1111C12 15.1838 15.1838 12 19.1111 12C23.0385 12 26.2222 15.1838 26.2222 19.1111Z"
                    stroke="#64696F"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center  rounded-lg py-2 px-4 font-medium text-sm gap-3.5 border border-solid border-[#D9D9D9]"
                >
                  <p>اضافه کردن</p>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button
                  onClick={() => {
                    setGolestanModal(true);
                  }}
                  className="flex bg-[#00ADB5] rounded-lg py-2 px-4 font-medium text-sm text-white gap-2.5"
                >
                  <p>آپدیت لیست</p>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
      ...
      <GolestanLoginModal
        onOk={() => handleOk()}
        onClose={() => setGolestanModal(false)}
        open={golestanModal}
      />
      <Modal
      ...
      >
        <div
          action=""
          dir="rtl"
          className="flex flex-col gap-5 h-full text-right mt-7 "
        >
          <div className="flex gap-6">
            <div className="flex flex-col ">
              <label for="unitName">عنوان درس</label>
              <input
          ...
          <div className="flex gap-6">
            <div className="flex flex-col flex-1 ">
              <label for="unitMaster">نام استاد</label>
              <input
                className="border border-solid border-[#A7A9AD] py-2 px-5 rounded-lg mt-2 placeholder:text-[#A7A9AD]"
                type="text"
                name="unitMaster"
                id="unitMaster"
                onChange={(e) => (professorName.current = e.target.value)}
                placeholder="امینی هرندی"
                required
              />
            </div>
            <div className="flex flex-col flex-1 ">
              <label for="name">کد درس</label>
              <input
                className="border border-solid border-[#A7A9AD] py-2 px-5 rounded-lg mt-2 placeholder:text-[#A7A9AD]"
                type="text"
                name="name"
          ...
          <div className="flex flex-col gap-2.5 mb-2.5">
            <div className="flex">
              <label for="name">زمان های ارائه درس</label>
            </div>
            ...
            <div className="">
              <button
                ...
                className="flex cursor-pointer items-center justify-center m-auto bg-transparent border-dashed p-1.5 rounded-lg border-[#D9D9D9]"
                type="button"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex">
              <label for="name">تاریخ امتحان </label>
            </div>
            <div className="flex  ">
              <div
                dir="ltr"
          ...
          <div className="w-full ">
            <button
              ...
              className="cursor-pointer w-full bg-[#4CC6CB] py-3 px-3 text-white outline-none border-none rounded-lg text-base font-bold"
              type="button"
            >
              افزودن
            </button>
