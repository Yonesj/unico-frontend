...
const ForgetpassModal = ({ open, onOk, onClose }) => {
  ...
  const isValidEmail = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(email); // Regex to check if the code is exactly 6 characters
  ...
  const handleSendEmail = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "ایمیل وارد شده نامعتبر است");
      }

      toast.open({
        message: "لینک تغیر گذرواژه به ایمیل شما ارسال شد",
        type: "success",
      });
